...
const CreateTicket = () => {
  ...
  const handleSubmit = async (data: z.infer<typeof FormSchema>) => {
    ...
    try {
      if (projects[0]?.projectName) {
        ...
        if (response.ok) {
          ...
          toast({
            ...
            description: <div>We will start working on it soon.</div>,
          });
        } else {
          toast({
            title: 'Error!',
            description: <div>There was an issue creating your ticket.</div>,
          });
        }
      }
    } catch (error) {
      ...
      toast({
        title: 'Error!',
        description: <div>We&apos;re experiencing an error. Please try again later.</div>,
      });
    }
  };
